...
class EShareBox extends CWidget
{
	...
	protected $shareDefinitions = array(
		'email' => array(
		'url' => 'http://tickgoals.com/site/shareMail?url={url}&title={title}',
		'title' => 'Email this',
		'name' => 'E-Mail',
		),
		'facebook' => array(
	...
	public function run()
	{
		...
		foreach ($this->shareDefinitions as $name => $def) {
			...
			$link = CHtml::link($linkText, $url, array('rel' => 'nofollow', 'target' => '_blank', 'id'=>'sharethis'.$name,'title' => $def['title']));
